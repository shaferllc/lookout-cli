...
#[AsCommand(name: 'unsnooze-error', description: 'Clear snooze for an error group')]
final class UnsnoozeErrorCommand extends AuthorizedCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $data = $this->client($input)->post('api/v1/errors/'.$eid.'/unsnooze', []);

        return $this->writeFormatted($input, $output, $data, function () use ($output): void {
            $output->writeln('<info>Snooze cleared.</info>');
        });
    }
}
